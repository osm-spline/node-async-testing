    ...
    var opts =
      ...
      , onCoverage: function(data) {
          ...
          for (var file in data) {
            if (coverage[file] === undefined) {
              coverage[file] = {};
            }

            for (var line in data[file]) {
              if (Number(line) == line) {
                if (coverage[file][line] === undefined) {
                  coverage[file][line] = 0;
                }

                coverage[file][line] += data[file][line];
              }
              else {
                if (line === 'source') {
                  for (var sourcelines in data[file][line]) {
                    data[file][line][sourcelines] =
                          data[file][line][sourcelines].replace(/&#37;/g, '%');
                  }
                }
                coverage[file][line] = data[file][line];
              }
